...
from qcli.xml_generator import make_xml

def parse_config_file(lines):
    """Parser for the Galaxy integration configuration file
...
def create_dirs(galaxy_dir, sections):
    """Create directories in the galaxy tool folder for all the sections defined
        in the configuration file

    Inputs:
        galaxy_dir: the full path to the Galaxy installation directory
        sections: a list with the different sections in which the scripts
            will be grouped by
...
def integrate(scripts_dir, galaxy_dist_dir, config_file, update_tool_conf, 
    ...
    Inputs:
        ...
        galaxy_dist_dir: path to the Galaxy's installation directory
        config_file: path to the Galaxy integration configuration file
        update_tool_conf: boolean showing if the current tool_conf.xml
            file should be updated or a new tool_conf.xml file should be created 
        log_fp: path where the log file should be written
    ...
    done according to the 'config_file', which specifies:
        - the different sections in which the scripts will be grouped
        - which scripts of the script folder will be integrated
        - which options of these scripts will NOT be included in the Galaxy
            interface.
    ...
    tool_conf.xml configuration file, built from scratch.
